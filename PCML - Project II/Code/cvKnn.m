% cvKnn - K-Nearest Neighbour classification
%
% Synopsis
...
%   (vector) [score]  1 x N vector containing the scores from the 
%                     classification.
...
% Modified Version
%   05/12/2014 by Angelopoulos Vasileios


function [Class, score, Rank] = cvKnn(X, Proto, ProtoClass, K, distFunc)
...
if K == 1, % sort distances only if K>1
    ...
    score = zeros( 1, size( maxi, 2 ) );
    % Compute thes scores that is associated with each prediction
    for i = 1 : size( maxi, 2 )
        if winnerLabelIndex(i) == 1
            score(i) = 1 - 2 * maxi(i) / K;
        else if winnerLabelIndex(i) == 2
                score(i) = -1 + 2 * maxi(i) / K;
            end
        end
    end
